...
function initAuth(){
  ...
  document.getElementById('registerBtn').addEventListener('click', ()=>{
    ...
    users[u] = { password: p, coins:0, unlockedTypes:{box:true}, carUnlocked:false, minigunUnlocked:false };
    saveUsers(users);
...
bombImg2.crossOrigin = 'anonymous';
...
minigunImg.crossOrigin = 'anonymous';
...
function loadPropTexture(name, filename){
  ...
  img.onload = () => {
    try{
      ...
      const tex = new Image(); tex.onload = ()=>{ propTextures[name]=tex; propTextureLoaded[name]=true }; tex.src = off.toDataURL();
    }catch(e){ console.warn('Error loading prop texture', name, e); propTextures[name]=img; propTextureLoaded[name]=true }
  };
  img.onerror = ()=>{ /* missing file ok */ };
}
